...
uint8_t cdc_line_coding[7] = {0,0,0,0,0,0,0};
...
middle:
...
#ifdef SUPPORT_SETSPEED
        // These two are not supported by snd driver, of course.
        // I keep this door open, yet provide small libusb utility to work like if we are regular USB-Serial, but, unlike other setters, this utility does not require /dev/tty... device, which we can't have at USB-MIDI.
        if (bRequest == 0x21 && bmRequestType == 0xA1) { // CDC_GET_LINE_CODING
            usb_wait_in_ready();
            uint8_t *p;
            p = cdc_line_coding;
            for (i=0; i<7; i++)
        ...
        if (bRequest == 0x20 && bmRequestType == 0x21) { // CDC_SET_LINE_CODING
            ...
            uint32_t bps = *(uint32_t *)cdc_line_coding;

            // 16MHz xtal supports only limited set of speeds. See also my other research about this exactly "misfeature" of atmega32u4.
            // Best way will be 1) read F_CPU (GET_... via libusb), 2) calc on host, 3) send divider and/or 4) warn user about impossible baud rate. But we conforms to de facto way of usb-serials setup, and it is via send of baudrate value as uint32, not the divisor. Sadly, it requires numeric division, which 8-bit platforms not in big love with. Later, using asm, this place will be rewritten.
            if ((bps < 57600) || (bps == 125000) || (bps == 250000) || (bps == 500000) || (bps >= 1000000))
            {
                UBRR1 = (F_CPU / 16 / bps) - 1;
                if (bps == 38400)
                    to_blink += 2;
            }
            else
                return; // I need to send error, will silence work?
