...
/*
USB FS to MIDI 1.0 Bridge & no-cable Echo (loopback), based on fast USB-Serial, for RTT measure and regular use.
Platform: CJMCU Beetle 16 MHz with h/w USB FS, but note its resonator is ceramic, use with caution.
Binary:   2,9 kb with -O2.
Speed:    According to usbmon, self delay is below 100 us, typ. 70 us, using JACK2, >15 yrs old CPU, 6.18.9 non-RT kernel, and low DSP load.
...
#define ASCII_PRODUCT        L"MIDI 1.0 Bridge & Echo"
...
#define TRANSMIT_FLUSH_TIMEOUT  5   // ms
...
static const uint8_t PROGMEM endpoint_config_table[] = {
    1, 0x81, EP_SIZE(TXSZ) | EP_DOUBLE_BUFFER, // EP_TYPE_BULK_IN
    1, 0x80, EP_SIZE(RXSZ) | EP_DOUBLE_BUFFER, // EP_TYPE_BULK_OUT
};

static const uint8_t PROGMEM device_descriptor[] = {
    18, 1, 0x00, 0x02, 0, 0, 0, EP0SZ, VID, PID, 0x01, 0x00, 1, 2, 3, 1,
};

// https://www.usb.org/sites/default/files/midi10.pdf Appendix B.
#define CFGSZ (9+9+9+9+7+6+6+9+9+9+5+9+5) // 0x0065
static const uint8_t PROGMEM config1_descriptor[CFGSZ] = {
    9, 2, LSB(CFGSZ), MSB(CFGSZ), 2, 1, 0, 0xC0, 50,
    9, 4, 0, 0, 0, 1, 1, 0, 0,
    9, 0x24, 1, 0, 1, 0x09, 0x00, 1, 1,
    9, 4, 1, 0, 2, 1, 3, 0, 0,
    7, 0x24, 1, 0, 1, 0x41, 0x00,  // 7+6+6+9+9+9+5+9+5 = 0x0041
...
static const struct descriptor_list_struct {
    ...
    {0x0301, 0x0409, (const uint8_t *)&string1, sizeof(ASCII_MANUFACTURER)},
    {0x0302, 0x0409, (const uint8_t *)&string2, sizeof(ASCII_PRODUCT)},
    {0x0303, 0x0409, (const uint8_t *)&string3, sizeof(ASCII_SERIALNUMBER)}
};
#define NUM_DESC_LIST (sizeof(descriptor_list)/sizeof(struct descriptor_list_struct))

// Variables - these are the only non-stack RAM usage
// zero when we are not configured, non-zero when enumerated
static volatile uint8_t usb_isconfigured=0;
...
// 0 if the USB is not configured, or the configuration number selected by the HOST
char m_usb_isconnected(void)
{
    return (char)usb_isconfigured;
}
...
ISR(USB_GEN_vect)
{
    ...
    if (intbits & (1<<EORSTI)) {
        ...
        UECONX = 1;
        UECFG0X = 0;
        UECFG1X = EP_SIZE(EP0SZ) | EP_SINGLE_BUFFER;
        UEIENX = (1<<RXSTPE);
        usb_isconfigured = 0;
    }
    if (intbits & (1<<SOFI)) {
...
// Is our RAM enough?
// For MIDI->USB. FIXME Will it work for large SysEx's?
uint8_t buf[4];
// For USB->MIDI, we need larger size,
// as it is speed conversion bottleneck.
// Also used for Echo.
static uint8_t uwptr = 0, irptr = 0;
uint8_t ringbuf[256];

static uint8_t PC = 0;
static uint8_t SysEx = 0;
static uint8_t stateTransTable[16+8] = {
    0,          /* 0 dummy */
    0,          /* 1 dummy */
    3,          /* 2->3 Note OFF (3) */
    2 | 0x80,   /* 3->2 */
...
int main(void)
{
    ...
    if (jumper == 0b10100000) { // No jumper
        ...
        // MIDI Echo.
        // Message length is 4 bytes max, except Sysex which are unlimited length.
...
//                uwptr &= TX_MASK; // We have exactly one byte size pointers.
        ...
        // Wait for actual end of transmission, to correctly turn LED off.
        if(UCSR1A & (1<<TXC1)) {
